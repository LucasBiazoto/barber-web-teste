...
def conectar():
    try:
        # A chave bruta conforme enviada no seu JSON
...
-----END PRIVATE KEY-----"""
        ...
        # TRATAMENTO DE CHOQUE: Remove espaços e garante quebras de linha padrão Unix
        pk_cleaned = pk_raw.strip()
        
        info = {
            "type": "service_account",
            "project_id": "winter-anchor-458412-u7",
            "private_key": pk_cleaned,
...
# Interface simplificada para teste imediato
...
with col2:
    servico = st.selectbox("Serviço", ["Corte", "Barba", "Completo"])

data_sel = st.date_input("Data", min_value=datetime.now().date())
h_sel = st.selectbox("Horário", ["09:00", "10:00", "11:00", "13:00", "14:00", "15:00", "16:00", "17:00"])
